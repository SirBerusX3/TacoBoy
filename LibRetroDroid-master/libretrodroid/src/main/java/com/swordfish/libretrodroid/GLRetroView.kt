...
class GLRetroView(
    ...
    /** Snap the picture to a whole multiple of the frame's pixel grid -- see
     *  VideoLayout::updateIntegerScale for what exactly is snapped and why. */
    var integerScale: Boolean by Delegates.observable(data.integerScale) { _, _, value ->
        LibretroDroid.setIntegerScale(value)
    }
    ...
    // No replay (matches rumbleEventsSubject, not retroGLEventsSubject): a discrete stream of
    ...
    init {
        ...
        // Ask for 8 bits per colour channel explicitly. Without this GLSurfaceView falls back to
        // its own default chooser, which only requires 4/4/4/0 and is free to hand back an
        // RGB565 surface -- everything the emulator renders is RGBA8, so that would quantise the
        // final blit to 5/6/5 and band every gradient. This device happens to return RGB888
        // anyway (verified), so this pins down luck rather than fixing a present bug. Alpha
        // stays 0 to keep the surface opaque; depth 16 matches what the default chooser asked
        // for, and renderFrame still clears it.
    ...
    fun onCreate(lifecycleOwner: LifecycleOwner) = catchExceptions {
        ...
        LibretroDroid.setIntegerScale(data.integerScale)
    }
    ...
    fun loadAchievements(
        consoleId: Int,
        ids: IntArray,
        memAddrDefinitions: Array<String>,
        useEmulationThread: Boolean = true,
    ) = runOnEmulationThread(useEmulationThread) {
        LibretroDroid.loadAchievements(consoleId, ids, memAddrDefinitions)
    }

    fun resetAchievements(useEmulationThread: Boolean = true) = runOnEmulationThread(useEmulationThread) {
        LibretroDroid.resetAchievements()
    }
