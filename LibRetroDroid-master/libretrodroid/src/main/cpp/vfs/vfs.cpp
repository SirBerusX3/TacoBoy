...
struct retro_vfs_file_handle* VFS::virtualOpen(const char *path, unsigned int mode, unsigned int hints) {
    ...
    // Deliberately not stream->fd = duplicateFD. retro_vfs_file_close_impl()
    // closes stream->fp (via fclose, which already closes the underlying fd)
    // and, separately, stream->fd if it's set. Setting both to the same fd
    // makes it close twice, which fdsan flags as a fatal double-close/abort
    // on devices that enforce it strictly. The plain (non-virtual) open path
    // in libretro-common's own vfs_implementation.c keeps fd/fp mutually
    // exclusive for the same reason; mirror that here.
    stream->fd = 0;
    stream->hints = hints;
    stream->size = size;
    stream->buf = nullptr;
