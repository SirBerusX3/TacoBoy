...
void ImageRendererES3::initializeTextures(unsigned int width, unsigned int height) {
    // Same teardown FramebufferRenderer now does, via the shared helper rather than an
    // open-coded loop -- this renderer already got it right, which is what made the missing
    // one next door obvious.
...
Renderer::PassData ImageRendererES3::getPassData(unsigned int layer) {
    ...
    // `layer` is unsigned, so `layer >= 0` was always true.
