...
public:
    ...
    /** Frees every framebuffer in a shader-pass chain. The vector owns the structs but not
     *  the GL objects they name, so dropping it alone leaks them. */
    static void deleteFramebuffers(std::unique_ptr<Framebuffers> data);
};
