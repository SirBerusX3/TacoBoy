...
void ES3Utils::deleteFramebuffers(std::unique_ptr<ES3Utils::Framebuffers> data) {
    if (data == nullptr) {
        return;
    }

    for (auto& framebuffer : *data) {
        deleteFramebuffer(std::move(framebuffer));
    }
}
...
std::unique_ptr<ES3Utils::Framebuffers> ES3Utils::buildShaderPasses(
    ...
    // `passes.size()` is unsigned: on an empty chain `size() - 1` wraps to SIZE_MAX rather
    // than skipping the loop, so bail out explicitly. Every shader currently defines at
    // least one pass, which is the only reason this has never fired.
    if (passes.empty()) {
        return result;
    }
