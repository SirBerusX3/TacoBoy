...
public:
    ...
    /**
     * Size of the texture [getTexture] actually returns, which is not always the size of the
     * frame in it. A renderer that uploads frames itself allocates exactly one frame, so the
     * two match and the default below is right. A hardware-render target does not: the core
     * draws into an FBO sized for the largest frame it ever declared, using the bottom-left
     * corner of it, so callers have to sample the sub-rect rather than the whole texture.
     */
    virtual std::pair<unsigned, unsigned> getTextureAllocationSize() {
        return { (unsigned) lastFrameSize.first, (unsigned) lastFrameSize.second };
    }
