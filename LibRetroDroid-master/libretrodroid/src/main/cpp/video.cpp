...
void Video::updateProgram() {
    ...
    std::for_each(shaders.passes.begin(), shaders.passes.end(), [&](const auto& item){
        ...
        shader.gTextureScaleHandle = glGetUniformLocation(shader.gProgram, "textureScale");
...
void Video::renderFrame() {
    ...
    // Integer scaling needs the frame's size in core pixels, and a PS1 changes it mid-game
    // when it switches video mode. Pushed from here rather than from a geometry-change hook
    // so it cannot go stale; the layout ignores a size it already has, so this is free.
    videoLayout.updateFrameSize((unsigned) getTextureWidth(), (unsigned) getTextureHeight());
    ...
    // How much of the renderer's texture the current frame occupies. It is the whole thing
    // for a renderer that uploads frames itself, and a bottom-left sub-rect for a
    // hardware-render target allocated at the core's maximum size.
    auto allocation = renderer->getTextureAllocationSize();
    float textureScaleX = allocation.first > 0 ? getTextureWidth() / (float) allocation.first : 1.0F;
    float textureScaleY = allocation.second > 0 ? getTextureHeight() / (float) allocation.second : 1.0F;
    ...
    for (int i = 0; i < shadersChain.size(); ++i) {
        ...
        // A copy, not the layout's array: these get scaled down to the used sub-rect.
        auto coordinates = videoLayout.getTextureCoordinates();
        for (size_t j = 0; j < coordinates.size(); j += 2) {
            coordinates[j] *= textureScaleX;
            coordinates[j + 1] *= textureScaleY;
        }
        ...
        // The *allocated* size, not the frame size. Every shader derives its sampling
        // coordinates as `coords * textureSize` and back again, so pairing the allocated size
        // with the scaled-down coordinates above leaves `screenCoords` in frame pixels exactly
        // as before, while every derived lookup lands correctly inside the larger texture.
        glUniform2f(shader.gTextureSizeHandle, (float) allocation.first, (float) allocation.second);

        // Only the multi-pass shaders read this, to undo the scaling when they sample
        // `previousPass` -- that one is a frame-sized intermediate, not the render target.
        glUniform2f(shader.gTextureScaleHandle, textureScaleX, textureScaleY);
...
void Video::updateIntegerScale(bool integerScale) {
    videoLayout.updateIntegerScale(integerScale);
}
...
void Video::initializeRenderer(RenderingOptions renderingOptions) {
    ...
    if (renderingOptions.hardwareAccelerated) {
        renderer = new FramebufferRenderer(
            renderingOptions.maxWidth,
            renderingOptions.maxHeight,
