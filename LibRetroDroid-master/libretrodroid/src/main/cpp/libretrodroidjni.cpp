...
JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_step(
    ...
    if (!triggeredAchievements.empty()) {
        jclass cls = env->GetObjectClass(glRetroView);
        jmethodID sendAchievementTriggeredMethodID = env->GetMethodID(cls, "sendAchievementTriggeredEvent", "(I)V");
        for (uint32_t achievementId : triggeredAchievements) {
            env->CallVoidMethod(glRetroView, sendAchievementTriggeredMethodID, (jint) achievementId);
        }
    }
}
...
JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_loadAchievements(
    JNIEnv* env,
    jclass obj,
    jint consoleId,
    jintArray ids,
    jobjectArray memAddrDefinitions
) {
    try {
        jsize count = env->GetArrayLength(ids);
        jint* idsData = env->GetIntArrayElements(ids, nullptr);

        std::vector<std::pair<uint32_t, std::string>> definitions;
        for (jsize i = 0; i < count; i++) {
            auto jMemAddr = (jstring) env->GetObjectArrayElement(memAddrDefinitions, i);
            definitions.emplace_back((uint32_t) idsData[i], JniString(env, jMemAddr).stdString());
        }

        env->ReleaseIntArrayElements(ids, idsData, JNI_ABORT);

        LibretroDroid::getInstance().loadAchievements((uint32_t) consoleId, definitions);
    } catch (std::exception &exception) {
...
JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_resetAchievements(
    JNIEnv* env,
    jclass obj
) {
    try {
        LibretroDroid::getInstance().resetAchievements();
    } catch (std::exception &exception) {
        LOGE("Error in resetAchievements: %s", exception.what());
        JavaUtils::throwRetroException(env, ERROR_GENERIC);
    }
}
...
JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_setIntegerScale(
        JNIEnv* env,
        jclass obj,
        jboolean integerScale
) {
    LibretroDroid::getInstance().setIntegerScale(integerScale);
}
