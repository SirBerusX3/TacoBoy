...
void VideoLayout::updateForegroundVertices() {
    ...
    // Snap down to a whole multiple of the frame's pixel grid, so one source pixel always
    // covers the same number of screen pixels and a shader's grid or mask comes out evenly
    // spaced. Vertical drives it and the horizontal follows by the same factor, preserving
    // the core's aspect ratio -- see updateIntegerScale in the header for why that is the
    // right axis to drive from. Computed on the unrotated extents, which is exact at the
    // rotation this app actually uses (0) and approximate otherwise.
    if (integerScale && frameHeight > 0 && screenHeight > 0) {
        float drawnHeightPx = scaleY * (float) screenHeight;
        float multiple = std::floor(drawnHeightPx / (float) frameHeight);

        // Below 1x the frame is taller than the space it has; keep the fractional fit rather
        // than overflow the viewport.
        if (multiple >= 1.0F) {
            float snapFactor = (multiple * (float) frameHeight) / drawnHeightPx;
            scaleX *= snapFactor;
            scaleY *= snapFactor;

            LOGD(
                "Integer scale: %d x %d frame at %.0fx (%.1f px tall before snapping)",
                frameWidth,
                frameHeight,
                multiple,
                drawnHeightPx
    ...
    float emptyVerticalSpace = viewportRect.getHeight() - scaleY;
    switch (viewportAlignment) {
        case V_ALIGN_TOP:
            viewportYOffset -= emptyVerticalSpace;
            break;
        case V_ALIGN_BOTTOM:
            viewportYOffset += emptyVerticalSpace;
            break;
...
void VideoLayout::updateViewportAlignment(unsigned int viewportAlignment) {
    LOGD("Updated viewport alignment to : %d", viewportAlignment);

    this->viewportAlignment = viewportAlignment;
    updateBuffers();
}

void VideoLayout::updateFrameSize(unsigned int frameWidth, unsigned int frameHeight) {
    if (this->frameWidth == frameWidth && this->frameHeight == frameHeight) {
        return;
    }

    LOGD("Updated frame size to: %d x %d", frameWidth, frameHeight);

    this->frameWidth = frameWidth;
    this->frameHeight = frameHeight;

    // Only integer scaling reads this, so nothing needs recomputing while it is off.
    if (integerScale) {
        updateBuffers();
    }
}

void VideoLayout::updateIntegerScale(bool integerScale) {
    if (this->integerScale == integerScale) {
        return;
    }

    LOGD("Updated integer scale to : %d", integerScale);
