...
void Environment::setLoadedContent(const std::string &path, const void *data, size_t size) {
    clearLoadedContent();
    ...
    gameInfoExtFullPath = path;

    // `path` here is a virtual filename rather than a real filesystem path, so all of this is
    // derived from the string itself rather than from anything on disk. `name` is what a core
    // builds its own save filenames from (Handy's .eeprom), so it is the basename with the
    // extension removed -- and `ext` is lower-cased, which libretro.h requires.
    auto lastSeparator = gameInfoExtFullPath.find_last_of("/" "\\");
    std::string baseName = lastSeparator == std::string::npos
        ? gameInfoExtFullPath
        : gameInfoExtFullPath.substr(lastSeparator + 1);
    gameInfoExtDir = lastSeparator == std::string::npos
        ? ""
        : gameInfoExtFullPath.substr(0, lastSeparator);

    auto lastDot = baseName.find_last_of('.');
    if (lastDot == std::string::npos) {
        gameInfoExtName = baseName;
        gameInfoExtExt = "";
    } else {
        gameInfoExtName = baseName.substr(0, lastDot);
        gameInfoExtExt = baseName.substr(lastDot + 1);
        std::transform(
            gameInfoExtExt.begin(),
            gameInfoExtExt.end(),
            gameInfoExtExt.begin(),
    ...
    gameInfoExt.data = data;
    gameInfoExt.size = size;
    gameInfoExt.file_in_archive = false;
    // The buffer is released once the game is unloaded rather than held to retro_deinit, so
    // a core that wants it beyond retro_load_game has to take its own copy.
    gameInfoExt.persistent_data = false;

    gameInfoExtValid = true;

    LOGD(
        "Published game info ext: name '%s', ext '%s', %zu bytes",
        gameInfoExtName.c_str(),
        gameInfoExtExt.c_str(),
        size
    );
}

void Environment::clearLoadedContent() {
    gameInfoExtValid = false;
    gameInfoExt = { };
    gameInfoExtFullPath.clear();
    gameInfoExtDir.clear();
    gameInfoExtName.clear();
    gameInfoExtExt.clear();
}
...
/** Deep-copies the descriptor array -- the core's own memory isn't guaranteed to outlive
 *  this call. Consumed by Achievements (achievements.cpp) via rc_libretro_memory_init to
 *  build the correct unified address space for live RetroAchievements tracking (e.g. GBA
 *  EWRAM+IWRAM) -- without this, only whatever single region
 *  retro_get_memory_data(RETRO_MEMORY_SYSTEM_RAM) covers is visible, which for mGBA is
 *  EWRAM alone, silently reading 0 for anything in IWRAM and causing achievement
 *  conditions that reference it to misfire. Confirmed live against a real device/account
 *  this session -- see CHANGELOG.md. */
bool Environment::environment_handle_set_memory_maps(const struct retro_memory_map* received) {
    memoryMapDescriptors.assign(received->descriptors, received->descriptors + received->num_descriptors);
    memoryMap.descriptors = memoryMapDescriptors.data();
    memoryMap.num_descriptors = memoryMapDescriptors.size();
    hasMemoryMap = true;
    return true;
}
...
bool Environment::handle_callback_environment(unsigned cmd, void *data) {
    switch (cmd) {
        ...
        case RETRO_ENVIRONMENT_GET_GAME_INFO_EXT:
            LOGD("Called RETRO_ENVIRONMENT_GET_GAME_INFO_EXT");
            // Only meaningful while a game is loaded, and only when the content actually
            // arrived as a memory buffer. A core told `false` here falls back to the path in
            // retro_game_info, which is exactly the pre-existing behaviour.
            if (!gameInfoExtValid) {
                return false;
            }
            *(const struct retro_game_info_ext **) data = &gameInfoExt;
            return true;
        ...
        case RETRO_ENVIRONMENT_SET_GEOMETRY: {
            ...
            // Fire synchronously, before returning control to the core, rather than polling
            // this flag after retro_run() returns. A HW-rendered core requests its current
            // framebuffer (get_current_framebuffer) and renders into it later in this same
            // retro_run() call, using the new size it just told us about here -- if the
            // renderer's FBO isn't resized until the *next* step(), the core ends up
            // rendering the new resolution into the still-old-sized buffer for this frame
            // (PS1 intro/logo screens rendering oversized and cropped under Beetle PSX HW's
            // hardware_gl renderer was exactly this).
            if (geometryChangedCallback != nullptr) {
                geometryChangedCallback(gameGeometryWidth, gameGeometryHeight);
            }
