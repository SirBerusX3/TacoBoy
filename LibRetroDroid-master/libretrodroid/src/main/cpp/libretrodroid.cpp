...
void LibretroDroid::callback_geometry_changed(unsigned width, unsigned height) {
    LibretroDroid::getInstance().handleGeometryChanged(width, height);
}
...
bool LibretroDroid::unserializeState(int8_t *data, size_t size) {
    ...
    // A save state can jump memory around discontinuously relative to whatever the
    // achievement runtime last saw (e.g. loading an earlier point after progress toward a
    // hit-count condition) -- clear that in-progress state rather than risk a spurious
    // trigger or corrupted hit count on the next frame. resetProgress() (rc_runtime_reset)
    // keeps every currently-active achievement activated, just clearing their hit-count/
    // delta state -- unlike the full reset() achievements.h warns against here, which would
    // deactivate everything and silently stop tracking for the rest of the session, since
    // nothing re-activates achievements after the initial per-game-load fetch. RA's own
    // client instead serializes runtime progress alongside the save state to avoid losing
    // in-progress hit counts at all; that isn't done here.
...
void LibretroDroid::onSurfaceCreated() {
    ...
    // Applied after construction rather than as a tenth constructor argument -- it is a
    // user setting that can also change while a game is running, so it needs the setter
    // path anyway and there is no point having two.
    video->updateIntegerScale(integerScale);
...
void LibretroDroid::create(
    ...
    Environment::getInstance().setGeometryChangedCallback(&callback_geometry_changed);
...
void LibretroDroid::loadGameFromPath(const std::string& gamePath) {
    ...
    // Some cores read their content only through GET_GAME_INFO_EXT and ignore
    // retro_game_info::data entirely (Handy does) -- publish it either way.
    Environment::getInstance().setLoadedContent(
        game_info.path != nullptr ? game_info.path : "",
        game_info.data,
        game_info.size
    );
...
void LibretroDroid::destroy() {
    ...
    Environment::getInstance().clearLoadedContent();
...
void LibretroDroid::step() {
    ...
    // Geometry changes (some games override it at runtime) are handled synchronously via
    // LibretroDroid::handleGeometryChanged, called from within the SET_GEOMETRY environment
    // callback during retro_run() above -- see environment.cpp's SET_GEOMETRY case for why
    // this can't be polled here after the fact.
...
// Called synchronously from the SET_GEOMETRY/SET_SYSTEM_AV_INFO environment callback, while
// still inside the core's retro_run() -- before it goes on to request the current framebuffer
// and render into it at the new size. See environment.cpp's SET_GEOMETRY case.
void LibretroDroid::handleGeometryChanged(unsigned width, unsigned height) {
    if (video) {
...
void LibretroDroid::loadAchievements(uint32_t consoleId, const std::vector<std::pair<uint32_t, std::string>>& idsAndMemAddrs) {
    if (achievements) {
        achievements->loadAchievements(core.get(), consoleId, idsAndMemAddrs);
    }
}

void LibretroDroid::resetAchievements() {
    if (achievements) {
        achievements->reset();
    }
}

std::vector<uint32_t> LibretroDroid::consumeTriggeredAchievements() {
    if (achievements) {
        return achievements->consumeTriggeredAchievements();
    }
    return {};
}
...
void LibretroDroid::setIntegerScale(bool integerScale) {
    this->integerScale = integerScale;

    if (video) {
        this->video->updateIntegerScale(integerScale);
    }
}
