...
void ImmersiveMode::renderToFramebuffer(
    uintptr_t texture,
    ...
    // Only this pass reads the game texture, so only this pass has to sample the sub-rect the
    // frame actually occupies. The blur passes below read this class's own framebuffers, which
    // are exactly the size they were drawn at, and keep sampling the full 0..1.
    GLfloat gameTextureCoords[12];
    for (int i = 0; i < 12; i += 2) {
        gameTextureCoords[i] = gTextureCoords[i] * textureScaleX;
        gameTextureCoords[i + 1] = gTextureCoords[i + 1] * textureScaleY;
    }
    ...
    glVertexAttribPointer(blurTextureCoordinatesHandle, 2, GL_FLOAT, GL_FALSE, 0, gameTextureCoords);
...
void ImmersiveMode::renderBackground(
    ...
    if (blendFramebufferCurrent == 0) {
        renderToFramebuffer(texture, framebufferVertices, textureScaleX, textureScaleY);
    }
